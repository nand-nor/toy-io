...
fn test_default_parking_10millis(criterion: &mut Criterion) {
    let rt = setup_default(Some(10), Some(10), Some(5), Some(0));
...
fn test_parking_50millis(criterion: &mut Criterion) {
    let rt = setup_default(Some(50), Some(50), Some(5), Some(0));
...
fn test_no_parking(criterion: &mut Criterion) {
    let rt = setup_default(None, None, Some(5), Some(0));
...
fn test_no_io_parking(criterion: &mut Criterion) {
    let rt = setup_default(None, Some(10), Some(5), Some(0));
...
fn test_no_exec_parking(criterion: &mut Criterion) {
    let rt = setup_default(Some(10), None, Some(5), Some(0));
...
criterion_group! {
    name = default_park_config;
    config = Criterion::default().significance_level(0.2).sample_size(10);
    targets = test_default_parking_10millis, test_no_parking
}

criterion_group! {
    name = no_parking_vs_long_parking;
    config = Criterion::default().significance_level(0.2).sample_size(10);
    targets = test_parking_50millis, test_no_parking
}
...
criterion_group! {
    name = compare_no_io_vs_no_exec;
    config = Criterion::default().significance_level(0.2).sample_size(10);
    targets = test_default_parking_10millis, test_no_exec_parking, test_no_io_parking
}

criterion_main!(
    default_park_config,
  //  longer_parking,
    compare_no_io_vs_no_exec,
   // no_parking_vs_long_parking
);